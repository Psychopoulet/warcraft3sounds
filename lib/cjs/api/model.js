...
class WarcraftSoundsModel {
    ...
    getRaces() {
        return new Promise((resolve, reject) => {
            this._db.all("SELECT code, name FROM races ORDER BY name;", (err, data) => {
                return err
                    ? reject(err)
                    : resolve(data.map((race) => {
                        return Object.assign(Object.assign({}, race), { "url": "/api/races/" + race.code });
                    }));
            });
        });
    }
    ...
    getCharacter(codeRace, code, notWorded = false) {
        return new Promise((resolve, reject) => {
            this._db.get(" SELECT characters.id, characters.code, characters.name" +
                " FROM characters" +
...
exports.default = WarcraftSoundsModel;
