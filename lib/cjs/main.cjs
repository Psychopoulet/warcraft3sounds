...
const mediaRoutes_1 = __importDefault(require("./api/mediaRoutes"));
// consts
...
Promise.resolve().then(() => {
    ...
    // generate server
    if (CONF.get("ssl")) {
        const keys = node_forge_1.pki.rsa.generateKeyPair(4096);
        ...
        const pemPrivateKey = node_forge_1.pki.privateKeyToPem(keys.privateKey);
